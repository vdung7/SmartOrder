...
public class SalesReportChart {
	...
	public SalesReportChart(int productID, List<FoodOrder> orders, int parentWidth) {
		this.orders = orders;
	...
	public JPanel createReportPanel() {
		...
		// create charts
		ChartPanel ordersChart = new ChartPanel(createBarChart(ordersTrend, "Orders Trend", "Date", "Orders"));
		ChartPanel salesChart = new ChartPanel(createBarChart(salesTrend, "Sales Trend", "Date", "Sales"));
		ChartPanel pieChart = new ChartPanel(createPieChart(productDistribution, "Rate Distribution"));
		...
		split.setDividerLocation( (parentWidth/3)*2 );
	...
	private JFreeChart createBarChart(CategoryDataset dataset, String title, String domainLabel, String rangeLabel) {
		// create the chart...
		...
		for (FoodOrder fo : orders) {
			for (OrderDetail od : fo.getDetails()) {
				...
				if (p.getProductID()==productID) {
					...
					try {
						ordersTrend.incrementValue(od.getQuantity(), name , sdf.format(fo.getOrderDate()));
						salesTrend.incrementValue(sale, name, sdf.format(fo.getOrderDate()));
					} catch (UnknownKeyException e) {
						ordersTrend.addValue(od.getQuantity(), name, sdf.format(fo.getOrderDate()));
						salesTrend.addValue(sale, name, sdf.format(fo.getOrderDate()));
					}
				}
				
				// update data set of Distribution chart
				try {
					double currentValue = productDistribution.getValue(name).doubleValue();
					productDistribution.setValue(name, currentValue + od.getQuantity());
				} catch (UnknownKeyException e) {
