...
public class TCPServer extends Thread{
	...
	private void processClientConnection(Socket client) throws Exception{
		...
		boolean login = false;
		String currentUser = "";
		//in this while we wait to receive request from client (it's an infinite loop)
			...
			if (request != null && messageListener != null) {
				...
				try {
					...
					case ClientRequest.LOGIN:
						Customer c = CustomerHelper.checkLogin(em, request.getUsername(), request.getPassword());
						if(c!=null){
							response.setUserDetail(c.toArray());
							response.setMenu( ProductHelper.getMenuByAvailable(em, true) );
							login=true;
					...
					case ClientRequest.REGISTER:
						boolean isReg = CustomerHelper.register(em, 
								...
								request.getAddress(), request.getFone(), request.getEmail());
						response.setRegistered(isReg);
						break;
					case ClientRequest.ORDER:
						if(login){
							OrderHelper.makeOrder(em, request.getShoppingCart(), currentUser);
							response.setOrdered(true);
						}
						break;
					case ClientRequest.VIEWDETAIL:
						if(login){
							response.setDetail(OrderHelper.getDetail(em, request.getOrderID()));
						}
						break;
					case ClientRequest.VIEWORDERS:
						if(login){
							response.setOrderIDs( OrderHelper.getOrdersInString(em, currentUser) );
						}
						break;
					}
					...
					output.flush();
					et.commit();
				} catch (Exception e) {	// more kinds of exception will be process here
					e.printStackTrace();
